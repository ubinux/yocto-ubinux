...
## 2. Features
### 2.1 Security Fixes
Following security issues have been fixed from ubinux-2021.11.
...
Run as root privilege the Toolchain installer at arbitrary directory, and Toolchain will be installed into /opt/ubinux/2021.12 directory.
...
`$ sudo sh ubinux-glibc-x86_64-meta-toolchain-[TYPE1]-toolchain-2021.12.sh`
...
`$ . /opt/ubinux/2021.12/environment-setup-[TYPE2]`
...
#### 4.2.1 pcp
When using dstat, the error message is as following:  
<pre>
root # dstat
...
### 4.3 Tools
#### 4.3.1 dnf-plugin-tui
- Issue  
When installing rootfs with tui, if there is a problem with the dependencies, the following error message will be reported：  
`Error: Goal could not find a solution.`    
`Error: Failed to call dnf tui`    
However, the error message cannot correctly prompt the dependency problem.
- Solution  
Using the RPM package provided by ubinux to install rootfs will not cause problems.
